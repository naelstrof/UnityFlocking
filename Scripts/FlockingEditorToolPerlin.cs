...
public class FlockingEditorToolPerlin : FlockingEditorTool {
    ...
    protected override void StartOperation() {
        ...
        var foliages = FlockingData.GetFoliagePack().GetFoliages();
        List<Foliage> fillers = new List<Foliage>(foliages.Where((f) => f.foliageType == Foliage.FoliageType.Filler));
        List<Foliage> spillers = new List<Foliage>(foliages.Where((f) => f.foliageType == Foliage.FoliageType.Spiller));
        List<Foliage> thrillers = new List<Foliage>(foliages.Where((f) => f.foliageType == Foliage.FoliageType.Thriller));
        biomes.Clear();
    ...
    protected override void PostPointOperate(Vector3 point, Vector3 normal, Plane realSurface, FlockData data) {
        ...
        if (noise*scale*Random.Range(0.25f,4f) < 0.33f) {
            desiredIndex = FlockingData.GetFoliagePack().GetFoliageIndexOf(biomes[biomeSelection].spiller);
            if (Random.Range(0f, 1f) < 0.02f) {
                desiredIndex = FlockingData.GetFoliagePack().GetFoliageIndexOf(biomes[biomeSelection].thriller);
            }
        } else {
            desiredIndex = FlockingData.GetFoliagePack().GetFoliageIndexOf(biomes[biomeSelection].filler);
